...
function ClientFormBody({ initial, onClose, onSaved }: FormProps) {
  const t = useT()
  ...
  const handleSave = async () => {
    if (!name.trim()) {
      setError(t.clientForm.nameError)
      return
    }
    ...
    const payload = {
      ...
      source: source || null,
      cashback_percent: parseFloat(cashbackPct) || 0,
    }

    const query = isEdit && initial
      ? localDb.from('clients').update(payload).eq('id', initial.id)
      : localDb.from('clients').insert({
          ...payload,
          cashback_balance: 0,
          is_archived: false,
          referrer_id: null,
    ...
    <div className="flex flex-col gap-4">
      <div>
        <label className="text-sm font-medium text-slate-700 dark:text-slate-300 block mb-2">
          {t.clientForm.typeLabel}
        </label>
        <div className="flex gap-2">
          {(['individual', 'company', 'agent'] as ClientType[]).map((clientType) => (
            <button
              key={clientType}
              type="button"
              onClick={() => setType(clientType)}
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        />
        <Input
          label={t.clientForm.telegramLabel}
          value={telegram}
          onChange={(e) => setTelegram(e.target.value)}
          placeholder="@username"
        />
        <Input
          label={t.clientForm.birthdayLabel}
          type="date"
          value={birthday}
          onChange={(e) => setBirthday(e.target.value)}
        />
        <Input
          label={t.clientForm.sourceLabel}
          value={source}
          onChange={(e) => setSource(e.target.value)}
          placeholder="Instagram"
        />
        <Input
          label={t.clientForm.cashbackLabel}
          type="number"
          value={cashbackPct}
...
export function ClientFormModal({ open, onClose, onSaved, initial }: Props) {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={isEdit ? t.clientForm.editTitle : t.clientForm.createTitle}
      className="max-w-lg"
    >
      {open ? (
        <ClientFormBody
          key={initial?.id ?? 'new-client'}
