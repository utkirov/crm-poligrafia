import { useState } from 'react'
import { useNavigate } from 'react-router-dom'
import { localDb } from '../lib/localDb'
import { useAuthStore } from '../store/authStore'
import { Button } from '../components/Button'
import { Breadcrumbs } from '../components/Breadcrumbs'
import { StepClient } from './order-create/StepClient'
import { StepOrder } from './order-create/StepOrder'
import { StepPayment } from './order-create/StepPayment'
import { INITIAL_FORM_STATE, type OrderFormState } from './order-create/types'
import { useUnsavedChanges } from '../hooks/useUnsavedChanges'
import { toastSuccess, toastError } from '../lib/toast'
import { useT } from '../i18n'
import { parsePriceInput } from '../utils/priceInput'
import {
  calculateCashbackBalanceAfterApply,
  calculateCashbackToApply,
  calculatePayableAmount,
  willPaymentsExceedPayableAmount,
} from '../utils/paymentUtils'

function isStep1Valid(form: OrderFormState) {
  if (form.selectedClient) return true
  return form.newClient.name.trim() !== ''
}

function isStep2Valid(form: OrderFormState) {
  return form.title.trim() !== '' && parsePriceInput(form.totalAmount) > 0
}

export function OrderCreatePage() {
  const t = useT()
  const navigate = useNavigate()
  const user = useAuthStore((state) => state.user)
  const [step, setStep] = useState(0)
  const [form, setForm] = useState<OrderFormState>(INITIAL_FORM_STATE)
  const [submitting, setSubmitting] = useState(false)
  const [errors, setErrors] = useState<string[]>([])
  const [isDirty, setIsDirty] = useState(false)

  useUnsavedChanges(isDirty && !submitting)

  const onChange = (patch: Partial<OrderFormState>) => {
    setForm((previous) => ({ ...previous, ...patch }))
    setIsDirty(true)
  }

  const steps = [
    { label: t.orderCreate.stepClient, key: 'client' },
    { label: t.orderCreate.stepOrder, key: 'order' },
    { label: t.orderCreate.stepPayment, key: 'payment' },
  ] as const

  const canGoNext = step === 0 ? isStep1Valid(form) : step === 1 ? isStep2Valid(form) : true

  const handleNext = () => {
    if (step < 2) {
      setStep((value) => value + 1)
    }
  }

  const handleBack = () => {
    if (step > 0) {
      setStep((value) => value - 1)
    }
  }

  const handleSubmit = async () => {
    setSubmitting(true)
    setErrors([])

    try {
      let clientId = form.selectedClient?.id ?? null
      const total = parsePriceInput(form.totalAmount)

      if (!clientId) {
        const fullName = `${form.newClient.name.trim()} ${form.newClient.lastName.trim()}`.trim()
        const { data: newClientData, error: clientError } = await localDb
          .from('clients')
          .insert({
            name: fullName,
            type: form.newClient.type,
            phone: form.newClient.phone || null,
            telegram: form.newClient.telegram || null,
            source: form.newClient.source || null,
            referrer_id: form.newClient.referrerId || null,
            cashback_balance: 0,
            cashback_percent: 5,
            is_archived: false,
          })
          .select()
          .single()

        const newClient = newClientData as { id: string } | null
        if (clientError || !newClient) {
          setErrors([`${t.orderCreate.errorClient}: ${clientError?.message ?? t.common.error}`])
          setSubmitting(false)
          return
        }

        clientId = newClient.id
      }

      const cashbackApplied = form.applyCashback && form.selectedClient
        ? calculateCashbackToApply(form.selectedClient.cashback_balance, total)
        : 0
      const payableAmount = calculatePayableAmount(total, cashbackApplied)
      const validPayments = form.payments.filter((payment) => payment.amount && payment.dueDate)
      const plannedPaymentsTotal = validPayments.reduce((sum, payment) => sum + parsePriceInput(payment.amount), 0)

      if (willPaymentsExceedPayableAmount(payableAmount, plannedPaymentsTotal)) {
        setErrors([t.orderCreate.paymentsExceedTotal])
        setSubmitting(false)
        return
      }

      let referrerCashback = 0

      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const referrer = (form.selectedClient as any)?.referrer as { cashback_percent?: number } | null
      if (form.selectedClient?.referrer_id && referrer?.cashback_percent && referrer.cashback_percent > 0) {
        referrerCashback = Math.round(total * referrer.cashback_percent / 100)
      }

      const { data: orderData, error: orderError } = await localDb
        .from('orders')
        .insert({
          client_id: clientId,
          manager_id: user!.id,
          title: form.title.trim(),
          description: form.description.trim() || null,
          status: 'new',
          priority: form.priority,
          is_urgent: form.isUrgent,
          deadline: form.deadline || null,
          total_amount: total,
          cost_price: parsePriceInput(form.costPrice),
          referrer_cashback: referrerCashback,
          cashback_applied: cashbackApplied,
        })
        .select()
        .single()

      const order = orderData as { id: string } | null
      if (orderError || !order) {
        setErrors([`${t.orderCreate.errorOrder}: ${orderError?.message ?? t.common.error}`])
        setSubmitting(false)
        return
      }

      if (cashbackApplied > 0 && form.selectedClient) {
        await localDb.from('clients').update({
          cashback_balance: calculateCashbackBalanceAfterApply(form.selectedClient.cashback_balance, total),
        }).eq('id', clientId)
        await localDb.from('cashback_transactions').insert({
          client_id: clientId,
          order_id: order.id,
          type: 'spent',
          amount: cashbackApplied,
        })
      }

      if (form.orderItems.length > 0) {
        await localDb.from('order_items').insert(
          form.orderItems.map((item) => ({
            order_id: order.id,
            service_id: item.serviceId || null,
            quantity: item.quantity ? Number(item.quantity) : null,
            price_per_unit: item.pricePerUnit ? parsePriceInput(item.pricePerUnit) : null,
            total_price: parsePriceInput(item.totalPrice),
            is_manual_price: !item.isAutoTotal,
          })),
        )
      }

      if (validPayments.length > 0) {
        await localDb.from('payments').insert(
          validPayments.map((payment) => ({
            order_id: order.id,
            amount: parsePriceInput(payment.amount),
            due_date: payment.dueDate,
            payment_type: payment.paymentType,
            is_paid: false,
          })),
        )
      }

      await localDb.from('order_timeline').insert({
        order_id: order.id,
        user_id: user!.id,
        event_type: 'created',
        description: t.orderCreate.timelineCreated.replace('{name}', user!.name),
      })

      setIsDirty(false)
      toastSuccess(t.orderCreate.createdSuccess)
      navigate(`/orders/${order.id}`, { replace: true })
    } catch (error) {
      toastError(t.orderCreate.unknownError)
      setErrors([`${t.common.error}: ${String(error)}`])
      setSubmitting(false)
    }
  }

  const stepSummary = [
    form.selectedClient?.name
      ?? (form.newClient.name ? `${form.newClient.name} ${form.newClient.lastName}`.trim() : null),
    form.title || null,
    null,
  ]

  return (
    <div className="flex flex-col min-h-full page-enter">
      <div className="px-4 md:px-6 py-4 border-b border-slate-200 bg-white dark:bg-slate-800 dark:border-slate-700">
        <Breadcrumbs items={[{ label: t.nav.orders, to: '/dashboard' }, { label: t.orders.newOrder }]} />
        <h1 className="text-xl font-bold text-slate-900 dark:text-slate-100 mt-2">{t.orderCreate.breadcrumb}</h1>
      </div>

      <div className="flex flex-1">
        <div className="w-56 shrink-0 border-r border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
          <div className="flex flex-col gap-4">
            {steps.map(({ label }, index) => {
              const done = index < step
              const active = index === step
              const accessible = index <= step

              return (
                <button
                  key={index}
                  type="button"
                  onClick={() => (accessible ? setStep(index) : undefined)}
                  className={`flex items-start gap-3 text-left transition-colors ${
                    accessible ? 'cursor-pointer' : 'cursor-default'
                  }`}
                >
                  <div
                    className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 border-2 mt-0.5 transition-colors ${
                      done
                        ? 'bg-green-500 border-green-500 text-white'
                        : active
                          ? 'bg-blue-600 border-blue-600 text-white'
                          : 'bg-white dark:bg-slate-700 border-slate-300 dark:border-slate-600 text-slate-400 dark:text-slate-500'
                    }`}
                  >
                    {done ? '✓' : index + 1}
                  </div>
                  <div>
                    <p
                      className={`text-sm font-medium ${
                        active
                          ? 'text-blue-600'
                          : done
                            ? 'text-slate-900 dark:text-slate-100'
                            : 'text-slate-400 dark:text-slate-500'
                      }`}
                    >
                      {label}
                    </p>
                    {done && stepSummary[index] ? (
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 truncate max-w-32">
                        {stepSummary[index]}
                      </p>
                    ) : null}
                  </div>
                </button>
              )
            })}
          </div>
        </div>

        <div className="flex-1 p-8 max-w-3xl">
          <div className="mb-6">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">
              {t.orderCreate.step} {step + 1}: {steps[step].label}
            </h2>
          </div>

          {step === 0 ? <StepClient form={form} onChange={onChange} /> : null}
          {step === 1 ? <StepOrder form={form} onChange={onChange} /> : null}
          {step === 2 ? <StepPayment form={form} onChange={onChange} /> : null}

          {errors.length > 0 ? (
            <div className="mt-4 bg-red-50 border border-red-200 text-red-700 rounded-lg p-4 text-sm">
              {errors.map((message, index) => (
                <p key={index}>{message}</p>
              ))}
            </div>
          ) : null}

          <div className="flex items-center justify-between mt-8 pt-6 border-t border-slate-200 dark:border-slate-700">
            <Button variant="secondary" onClick={handleBack} disabled={step === 0}>
              {t.common.back}
            </Button>

            {step < 2 ? (
              <Button onClick={handleNext} disabled={!canGoNext}>
                {t.common.next}
              </Button>
            ) : (
              <Button
                variant="success"
                size="lg"
                onClick={handleSubmit}
                loading={submitting}
                disabled={!canGoNext}
              >
                {t.orderCreate.breadcrumb}
              </Button>
            )}
          </div>
        </div>
      </div>
    </div>
  )
}
